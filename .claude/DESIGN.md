...
나는 지금 클로드에서 PPT를 만들기 위한 프롬포트이자 디자인 시스템 설정 프롬ㅍ트를 쓰는 중이야
아래 내 요청을 반영해서 프롬포트를 수정해줘

폰트는 오직 맑은 고딕만 사용하고 다른 폰트는 안씀
나는 오직 16:9 슬라이드만 뽑음
최대한 쳅터명, 제목, 부제목이 같은 위치에 있기를 희망함
본문 하단을 텅텅 비워놓지 말고 밀도있게, 디자인에 악영향을 끼치지않는 선에서 채워주며 ㄴ좋곘어
